      ...
      <div className="hidden sm:flex items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            onBlur={() => onChange({ ...filter, search: searchInput, page: 1 })}
            onKeyDown={(e) => e.key === 'Enter' && onChange({ ...filter, search: searchInput, page: 1 })}
            className={`w-full pl-9 pr-3 py-2.5 rounded-xl ${inputClass}`}
            style={inputStyle}
          />
        </div>
        ...
        <div className="relative flex-shrink-0" ref={desktopDropdownRef}>
          <button
            type="button"
            className="relative flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm transition"
            style={{
              background: desktopFilterOpen || hasDropdownFilters ? 'rgba(167,139,250,0.12)' : 'rgba(255,255,255,0.05)',
              border: desktopFilterOpen || hasDropdownFilters ? '1px solid rgba(167,139,250,0.35)' : '1px solid rgba(255,255,255,0.1)',
              color: desktopFilterOpen || hasDropdownFilters ? '#C4B5FD' : 'rgba(248,250,252,0.55)',
            }}
          ...
          {desktopFilterOpen && (
            <div
              className="absolute right-0 top-full mt-2 z-30 w-80 rounded-2xl flex flex-col gap-5 p-5"
              style={{
                background: 'rgba(12, 12, 28, 0.98)',
                backdropFilter: 'blur(24px)',
                WebkitBackdropFilter: 'blur(24px)',
            ...
            >
              ...
              <div>
                ...
                <button
                  type="button"
                  onClick={() => { setDesktopFilterOpen(false); onOpenCategoryModal(); }}
                  className="w-full flex items-center justify-between gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors"
                  style={{
                    background: 'rgba(167,139,250,0.07)',
                    border: '1px solid rgba(167,139,250,0.2)',
                    color: '#C4B5FD',
              ...
              {availableModelTypes(filter.category).length > 0 && (
                <div>
                  ...
                  <select
                    ...
                    disabled={!filter.model_type}
                    className={`w-full px-3 py-2.5 rounded-xl ${inputClass} appearance-none cursor-pointer disabled:opacity-35 disabled:cursor-not-allowed`}
                    style={inputStyle}
                    aria-label="Subtyp"
                  >
                    <option value="" style={{ background: '#0f0f23' }}>Alle Subtypen</option>
