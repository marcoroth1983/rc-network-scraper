...
from app.analysis.job import BATCH_SIZE, run_analysis_job, recalculate_price_indicators
...
# ---------------------------------------------------------------------------
# Tests for recalculate_price_indicators (new similarity-based version)
# ---------------------------------------------------------------------------

def _make_orm_listing(
    lid: int,
    price_numeric: float,
    model_type: str | None = "airplane",
    manufacturer: str | None = None,
    model_subtype: str | None = None,
    completeness: str | None = None,
    attributes: dict | None = None,
) -> MagicMock:
    """Return a mock ORM Listing suitable for the recalculate logic."""
    m = MagicMock()
    m.id = lid
...
def _build_recalc_session_mocks(listings: list) -> tuple[MagicMock, MagicMock]:
    """Return (fetch_ctx, update_ctx) for the two AsyncSessionLocal() calls in recalculate_price_indicators."""
    fetch_session = AsyncMock()
    scalars_result = MagicMock()
    scalars_result.scalars.return_value.all.return_value = listings
    fetch_session.execute.return_value = scalars_result
...
@pytest.mark.asyncio
class TestRecalculatePriceIndicators:
    async def test_similar_cluster_gets_price_indicator(self) -> None:
        """5 similar listings of same manufacturer, tight price range → indicators assigned."""
        listings = [
            _make_orm_listing(1, 500.0, manufacturer="Multiplex", model_subtype="thermal", completeness="RTF"),
            _make_orm_listing(2, 450.0, manufacturer="Multiplex", model_subtype="thermal", completeness="RTF"),
            _make_orm_listing(3, 480.0, manufacturer="Multiplex", model_subtype="thermal", completeness="RTF"),
            _make_orm_listing(4, 510.0, manufacturer="Multiplex", model_subtype="thermal", completeness="RTF"),
            _make_orm_listing(5, 520.0, manufacturer="Multiplex", model_subtype="thermal", completeness="RTF"),
        ]
        fetch_ctx, update_ctx = _build_recalc_session_mocks(listings)
        call_count = 0
        ...
        with patch("app.analysis.job.AsyncSessionLocal", side_effect=_ctx_factory):
    ...
    async def test_listing_without_model_type_gets_null_indicator(self) -> None:
        """Listing without model_type is skipped — update is still called with NULL indicator."""
        listings = [
            _make_orm_listing(10, 300.0, model_type=None, manufacturer="Robbe"),
        ]
        fetch_ctx, update_ctx = _build_recalc_session_mocks(listings)
        call_count = 0
        ...
        update_session = await update_ctx.__aenter__()
        # An update must still be issued (to NULL out any stale indicator)
        assert update_session.execute.called

    async def test_heterogeneous_cluster_gets_null_indicator(self) -> None:
        """Wide price spread → heterogeneous cluster → price_indicator should be NULL."""
        # 5 listings with 10× price spread — ensures heterogeneous
        listings = [
            _make_orm_listing(20, 100.0, manufacturer="CARF", model_subtype="jet", completeness="ARF"),
            _make_orm_listing(21, 200.0, manufacturer="CARF", model_subtype="jet", completeness="ARF"),
            _make_orm_listing(22, 500.0, manufacturer="CARF", model_subtype="jet", completeness="ARF"),
            _make_orm_listing(23, 800.0, manufacturer="CARF", model_subtype="jet", completeness="ARF"),
            _make_orm_listing(24, 1000.0, manufacturer="CARF", model_subtype="jet", completeness="ARF"),
        ]
        fetch_ctx, update_ctx = _build_recalc_session_mocks(listings)
        call_count = 0
        ...
        update_session = AsyncMock()
        update_session.execute = AsyncMock(side_effect=lambda *a, **kw: captured_calls.append(a))

        update_ctx2 = MagicMock()
        update_ctx2.__aenter__ = AsyncMock(return_value=update_session)
        update_ctx2.__aexit__ = AsyncMock(return_value=False)
        ...
        def _ctx_factory2():
            ...
            call_count2 += 1
            return fetch_ctx if call_count2 == 1 else update_ctx2

        with patch("app.analysis.job.AsyncSessionLocal", side_effect=_ctx_factory2):
            await recalculate_price_indicators()

        # All updates should have ind=None (NULL indicator)
        # The execute calls include the UPDATE statement with {"ind": None, ...}
        assert update_session.commit.called

    async def test_idempotent_second_run_same_result(self) -> None:
        """Calling recalculate twice produces the same set of update calls."""
        listings = [
            _make_orm_listing(30, 300.0, manufacturer="Multiplex"),
            _make_orm_listing(31, 320.0, manufacturer="Multiplex"),
        ]
        ...
        for _ in range(2):
            ...
            update_session = AsyncMock()
            update_session.execute = AsyncMock(side_effect=lambda *a, **kw: captured.append(kw))
            update_ctx = MagicMock()
            update_ctx.__aenter__ = AsyncMock(return_value=update_session)
            update_ctx.__aexit__ = AsyncMock(return_value=False)
            ...
            def _factory(fc=fetch_ctx, uc=update_ctx):
                def _inner():
                    nonlocal call_n
                    call_n += 1
                    return fc if call_n == 1 else uc
                return _inner

            with patch("app.analysis.job.AsyncSessionLocal", side_effect=_factory()):
                await recalculate_price_indicators()

            results.append(captured)

        # Both runs should have performed the same number of update calls
        assert len(results[0]) == len(results[1])

    async def test_run_analysis_job_no_longer_calls_recalculate(self) -> None:
        """After PLAN-020, run_analysis_job must NOT call recalculate_price_indicators."""
        listing = _make_listing_orm(listing_id=99)
        analysis = _non_empty_analysis()
        fetch_ctx, update_ctx = _make_session_ctx([listing])
        call_count = 0
        ...
        with patch("app.analysis.job.settings") as mock_settings:
            ...
            with patch("app.analysis.job.AsyncSessionLocal", side_effect=_make_ctx):
                with patch("app.analysis.job.analyze_listing", new_callable=AsyncMock, return_value=analysis):
                    with patch("app.analysis.job.asyncio.sleep", new_callable=AsyncMock):
                        with patch(
                            "app.analysis.job.recalculate_price_indicators",
                            new_callable=AsyncMock,
                        ) as mock_recalc:
                            await run_analysis_job()
                            # recalculate must NOT be called from run_analysis_job anymore
