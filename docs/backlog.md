...
- **PLAN033-FE-NITS: Frontend-Review Cycle-1 Restpunkte (nicht-blockierend)** — Zurückgestellt aus PLAN_033 Frontend-Review (2026-06-14), bewusst klein gehalten (Single-User-Hobby): (M3) `MiniChart` Single-Point-Line erzeugt entartete unsichtbare Polyline — tritt praktisch nicht auf, da Timeseries durch Zero-Fill immer `days≥7` Punkte liefert; (L1) `deleteUser` verwirft Backend-`detail` bei Fehler (zeigt nur `HTTP <status>`); (L2/L3) Range-Selector-Buttons + „← Dashboard"-Link ohne `focus-visible`-Ring; (S2) `preserveAspectRatio="none"` verzieht Balken-/Punktpositionen bei sehr schmaler Breite; (S3) `cardStyle`-Konstante in `MetricsPanel` + `UserApprovalPanel` dupliziert → in shared util extrahierbar. _Aus PLAN_033 Review Cycle 1._
...
- ~~**DEPLOY-01: Deploy-Workflow liefert False-Green — `no such service: admin` bricht den Pull ab.**~~ **Fixed in PLAN-037 (2026-07-23):** `docker-compose.prod.yml` wird nun via `appleboy/scp-action` vor dem SSH-Deploy auf den VPS kopiert; `IMAGE_TAG=<sha>` pinnt backend+nginx atomisch; `set -euo pipefail` verhindert falsches Grün; per-Service-Digest-Gate (`running==pulled`) beweist, dass ein echter Deploy stattgefunden hat. _v2.9.1 wurde am 2026-07-17 manuell nachgezogen._

- **DEPLOY-03: Backend-Image-Versioning — globales `IMAGE_TAG` vs. per-Service-Pin bestätigen.** Bis PLAN-037 war das Backend manuell auf `d4c8961` gepinnt; ab PLAN-037 pinnen backend+nginx atomisch via `IMAGE_TAG=<short-sha>` pro Release. Bestätigen, dass dieses globale Modell (beide Services immer gleicher SHA) gegenüber per-Service-Pins gewünscht ist — nach dem ersten PLAN-037-Release prüfen. _Erstellt PLAN-037 (2026-07-23)._

- ~~**DEPLOY-04: Tote Subdomain `admin.rcn-scout.d2x-labs.de` liefert 502.**~~ **rcn-seitig NICHTS zu tun (verifiziert 2026-07-23).** Die alte Route ist vollständig entfernt: kein Traefik-Router (Traefik nutzt nur Docker-Labels, `exposedbydefault=false`; kein Container trägt eine `admin.rcn-scout`/`rcn-admin`-Regel), kein Container, kein Compose-Service, kein Code. Es gibt auch **keinen spezifischen DNS-Record zum Löschen** — `admin.rcn-scout` kommt über das geteilte **Wildcard `*.d2x-labs.de → 152.53.238.3`** (`INFRASTRUCTURE.md`). Das 502 ist **generisches Traefik-Verhalten für JEDEN nicht-konfigurierten Host** unterm Wildcard (verifiziert: `totally-made-up-9271.d2x-labs.de` und `foo.bar.d2x-labs.de` liefern identisch 502; nur Hosts mit echtem Router → 200). → **Kein rcn-Artefakt.** Optionaler Infra-Wunsch (NICHT rcn-scoped): ein globaler Traefik-Default-Catch-all-Router (→ sauberes 404 statt 502) betrifft ALLE d2x-Projekte (mfc-bussard, immo-calculator, cockpit, tradecore …) und ist eine geteilte-Infra-Entscheidung, nicht Teil von rcn. _Fehldiagnose „DNS-Record löschen" oben korrigiert._

- ~~**DEPLOY-02: Admin-Console (PLAN-034) auf Prod ausrollen**~~ — **VERWORFEN (Entscheidung 2026-07-17).** Die standalone Admin-Console wird **nicht** deployed. Das Admin-/Analytics-Panel wird Teil des zentralen **`d2x-control-plane`**-Cockpits für alle d2x-Apps (Umami läuft dort bereits). Die `admin/`-SPA ist eingefroren; ein späterer Plan entfernt sie, sobald das Cockpit ihre Features abdeckt. Der `admin`-Service in `docker-compose.prod.yml` bleibt definiert-aber-undeployed. Auth-Neudesign (App-lokales Subdomain-Cookie-SSO generalisiert NICHT über mehrere App-Domains → zentraler IdP oder Service-Token/Read-API je App nötig) ist ein `d2x-control-plane`-Thema und muss dort gebrainstormt werden. Details: `limitations.md` → „Standalone Admin Console built but frozen". _Damit ist der Deploy-Fix (DEPLOY-01) rein pipeline-only; Admin bleibt raus._

## PLAN-034 Auth — deferred review items (backend-relevant, SPA retired)

_From PLAN-034 cycle-1 python review (2026-06-18). The standalone admin SPA was retired in PLAN-037; items specific to the deleted `admin/` frontend code are closed. Backend items remain relevant to `backend/app/api/auth.py`:_
