...
- **DEPLOY-04: Tote Subdomain `admin.rcn-scout.d2x-labs.de` liefert 502.** Nach dem Admin-Retire (PLAN-037) serviert die Subdomain keine rcn-Admin-App mehr (korrekt), gibt aber statt eines sauberen Nichts ein **502** zurück — es existiert offenbar noch ein DNS-A-Record (→ VPS) ohne zugehörigen Traefik-Router/Service. Rein kosmetisch (Subdomain wird nicht genutzt; das Cockpit läuft auf `admin.d2x-labs.de`). Cleanup: DNS-A-Record für `admin.rcn-scout` entfernen (Netcup) und ggf. verwaiste Traefik-Route prüfen. _Beobachtet nach PLAN-037-Deploy (2026-07-23)._
